...
CHARACTER(LEN=*), PARAMETER :: infile = 'adios_test.in'
...
CHARACTER(LEN=nf90_max_name) :: varname_adios
CHARACTER(LEN=file_len) :: adios_config
CHARACTER(LEN=file_len) :: adios_outfile

TYPE(adios2_adios) :: adios
TYPE(adios2_io) :: ioPut
TYPE(adios2_variable) :: var_adios
TYPE(adios2_variable), ALLOCATABLE :: var_adios_copy(:)
TYPE(adios2_attribute) :: lat_att,lon_att,lev_att
...
SUBROUTINE adios_init(do_ens,ensemble_member,adios_config)

IMPLICIT NONE

LOGICAL, INTENT(IN) :: do_ens
INTEGER(KIND=INT32), INTENT(IN) :: ensemble_member
CHARACTER(LEN=*), INTENT(IN) :: adios_config
...
CALL adios2_init(adios,adios_config,comm,ierror)
CALL adios2_declare_io(ioPut,adios,'adios_test',ierror)
...
SUBROUTINE adios_def()

IMPLICIT NONE

INTEGER(KIND=INT32), PARAMETER :: ndims = 3

INTEGER(KIND=INT32) :: ierror
INTEGER(KIND=INT32) :: i
INTEGER(KIND=INT64) :: ishape(ndims), istart(ndims), icount(ndims)

CHARACTER(LEN=nf90_max_name) :: varname_adios_copy

CALL get_timing('Start of adios_def')

ishape(1) = nlon_in
ishape(2) = nlat_in
ishape(3) = nlev_in

istart(1) = start_in_x(iprocx)-1
istart(2) = start_in_y(iprocy)-1
istart(3) = 0

icount(1) = size_in_x(iprocx)
icount(2) = size_in_y(iprocy)
icount(3) = nlev_in

CALL adios2_define_variable(var_adios,ioPut, &
                            varname_adios,adios2_type_dp, &
                            ndims,ishape,istart,icount, &
                            adios2_constant_dims,ierror)

IF (ncopies > 0) THEN

  ALLOCATE(var_adios_copy(ncopies))

  DO i=1,ncopies
    WRITE(varname_adios_copy,'(A,I0)') TRIM(varname_adios)//'_copy_',i
    CALL adios2_define_variable(var_adios_copy(i),ioPut, &
                                varname_adios_copy,adios2_type_dp, &
                                ndims,ishape,istart,icount, &
                                adios2_constant_dims,ierror)
  END DO
END IF

CALL adios2_define_attribute(lon_att,ioPut,'longitude',lon_in,nlon_in,ierror)
CALL adios2_define_attribute(lat_att,ioPut,'latitude',lat_in,nlat_in,ierror)
CALL adios2_define_attribute(lev_att,ioPut,'p',lev_in,nlev_in,ierror)

CALL get_timing('End of adios_def')

END SUBROUTINE adios_def

!-------------------------------------------------------------------------------

SUBROUTINE adios_open_file()

IMPLICIT NONE

INTEGER(KIND=INT32) :: ierror

!CALL adios2_set_engine(ioPut, 'HDF5', ierror)

CALL adios2_open(bpWriter, ioPut, adios_outfile, adios2_mode_write, ierror)

END SUBROUTINE adios_open_file

!-------------------------------------------------------------------------------

SUBROUTINE adios_close_file()

IMPLICIT NONE

INTEGER(KIND=INT32) :: ierror

call adios2_close(bpWriter, ierror)

END SUBROUTINE adios_close_file

!-------------------------------------------------------------------------------

SUBROUTINE adios_final()
